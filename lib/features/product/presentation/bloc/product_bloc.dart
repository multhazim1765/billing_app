...
class ProductBloc extends Bloc<ProductEvent, ProductState> {
  ...
  final SeedDefaultProductsUseCase seedDefaultProductsUseCase;
  ...
  ProductBloc({
    ...
    required this.seedDefaultProductsUseCase,
    required this.updateProductUseCase,
    required this.deleteProductUseCase,
  }) : super(const ProductState()) {
    on<LoadProducts>(_onLoadProducts);
    on<AddProduct>(_onAddProduct);
    on<SeedDemoProducts>(_onSeedDemoProducts);
  ...
  Future<void> _onSeedDemoProducts(
      SeedDemoProducts event, Emitter<ProductState> emit) async {
    emit(state.copyWith(status: ProductStatus.loading));
    final result = await seedDefaultProductsUseCase(NoParams());
    result.fold(
      (failure) => emit(state.copyWith(
          status: ProductStatus.error, message: failure.message)),
      (count) {
